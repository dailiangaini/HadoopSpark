...
```
顶级纬度信息类：public abstract class BaseDimension implements WritableComparable<BaseDimension>

    浏览器纬度：BrowserDimension extends BaseDimension 【id, browserName 名称, browserVersion 版本】
    时间维度: DateDimension extends BaseDimension【id，year 年， season 季度， month 月，week 周，day 天，type 类型(按照年、周...分析)】
    平台维度：PlatformDimension extends BaseDimension 【id，platformName 平台名称】
    模块纬度：KpiDimension extends BaseDimension【id，kpiName 模块】
    地点纬度：LocationDimension extends BaseDimension【id， country 国家，province 省， city 城市】
    InboundDimension extends BaseDimension【id，parentId，name，hostRegex，type】

组合纬度

    基础组合纬度：public abstract class StatsDimension extends BaseDimension
        
        公用的信息组合: StatsCommonDimension extends StatsDimension【DateDimension，PlatformDimension，KpiDimension】
        进行用户分析(用户基本分析和浏览器分析)定义的组合维度: StatsUserDimension extends StatsDimension【StatsCommonDimension， BrowserDimension】
        统计location相关的维度类:StatsLocationDimension extends StatsDimension【StatsCommonDimension，LocationDimension】
        统计inbound相关信息时候维度类: StatsInboundDimension extends StatsDimension【StatsCommonDimension，InboundDimension】
        统计inbound的跳出会话维度类: StatsInboundBounceDimension extends StatsDimension【StatsCommonDimension，sid，serverTime】
```
# 4. 输出值类设计

```
自定义顶级的输出value父类：public abstract class BaseStatsValueWritable implements Writable 【public abstract KpiType getKpi】
    
    定义一系列的字符串输出类：TextsOutputValue extends BaseStatsValueWritable【kpiType，uuid 用户唯一标识符，sid 会话id】
    时间值输出类TimeOutputValue extends BaseStatsValueWritable【id，time 时间戳】
    MapWritableValue extends BaseStatsValueWritable【MapWritable，KpiType】
    LocationReducerOutputValue extends BaseStatsValueWritable【KpiType，uvs 活跃用户数，visits 会话个数，bounceNumber 跳出会话个数】
    InboundReduceValue extends BaseStatsValueWritable【KpiType，uvs，visit】
    InboundBounceReduceValue extends BaseStatsValueWritable【KpiType，bounceNumber】
```
# 5. 参数分析

```
1.新用户==》EventEnum.LAUNCH(1, "launch event", "e_l"), // launch事件，表示第一次访问
    即用户第一次访问，即为新用户

2.活跃用户==》PAGEVIEW(2, "page view event", "e_pv"), // 页面浏览事件
    访问页面的用户
```
